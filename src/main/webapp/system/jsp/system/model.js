...
function initModeltree(){
	  var modelTree = $("#modelTree");
	  var fakeNodeId = "fakeNode";//假节点id。
	  var rootId = "";
	  
	  //设置根节点及模块树的属性。
	  modelTree.tree({
	        data: [{id:rootId,text:"MavenDemo系统",state:'closed',attributes:{modelstatus:'N',modeltype:'0'}}],
	  });
	  //设置获取子节点的url。
	  modelTree.tree('options').url = ctx + "/permission/querryModelByParentId";
	    
	  //查出第一层子节点。
	  var node = modelTree.tree('find', rootId);
	  if (node) {
	       modelTree.tree('expand', node.target);
	  }
}
